...
- **Typed failures, top to bottom** — every failure a user can reach raises a `GenesisError` subclass rather than leaking a stdlib exception, so a missing template names the reinstall, an exhausted stdin names the argument to pass, an unreachable local model names the command that starts it, and Ctrl-C exits 130 instead of printing a traceback. (Argument validation on the library path still raises `ValueError`, which is what Python means by it.) The package ships a `py.typed` marker, and `mypy` runs in CI **against the real Anthropic SDK**, so a change to the SDK's parameters fails the build rather than reaching users.
...
- **The planner's contract is enforced at runtime, not by the schema** — deliberately. A flat schema cannot express *"if the status is `ready`, a plan must be present"*; a tagged union can, and [was measured doing so](https://github.com/jborrajo21/genesis/blob/main/EVAL.md) on both backends. It is not shipped because **schema branch order was found to change what the model generates**, not just what validates, so adopting it would confound the next eval run. Genesis raises a typed error instead: the invalid response is caught rather than made unreachable, and the contract lives in two places on purpose.
