...
- **Message reworded by D-090 (Sept 21, 2026):** it named a CLI flag the hosted API does not have. The fail-fast choice recorded here is unchanged.
...
## D-086: The deploy's cost guardrails — no reserved concurrency, and 128 MB sized from measurement

- **Context:** Task 4 went live Sept 21, 2026 at `https://<id>.lambda-url.us-east-2.on.aws/`, arm64, public (`AuthType: NONE`). A keyless public endpoint's only real exposure is compute, so the guardrails are the deploy, not an afterthought to it. Two of the three planned ones survived contact; the third could not exist at all.

- **`ReservedConcurrentExecutions` is impossible on this account, not merely omitted.** The first `sam deploy` failed with `decreases account's UnreservedConcurrentExecution below its minimum value of [10]`. Measured cause: `aws lambda get-account-settings` reports `ConcurrentExecutions: 10` — a new-account restriction, not the usual 1000 — and Lambda requires unreserved concurrency to stay at or above 10. **Any** reservation is therefore rejected, including 1. The account quota *is* the cap instead: nothing can exceed 10 concurrent invocations.
- **This weakens D-082 on one specific point, and the entry should be read with that in mind.** D-082's decisive argument was that guardrails become *verifiable* because a reader can check them in the tracked template. The concurrency cap is now an invisible account setting that no reader of the repo can see. The mitigation is a comment in `template.yaml` where the line would have gone, naming the reason and the Service Quotas path to restore it. **If the quota is ever raised, the line goes back** — not for more concurrency, which is not wanted, but so the cap is expressed where it can be reviewed.

- **`MemorySize: 128`, chosen from measurement rather than estimate.** Measured `Max Memory Used: 41 MB` against the initial 1024 — 25× over-provisioned. The binding trade-off was not normal-traffic cost, which is a fraction of a cent at any setting, but **worst-case exposure against cold-start latency**, since concurrency cannot be reserved and near-zero traffic makes cold start the common path rather than the rare one.
- **The reasoning that decided it:** at 128 MB a month of *continuously pegged* abuse costs roughly $55, which stays inside the $71.11 of credits expiring Dec 16 — the only setting where the worst case cannot reach a payment card. Every higher setting (512 ≈ $220/mo, 1024 ≈ $440/mo) puts the worst case beyond the credits and leaves a manually-actioned budget alert as the sole backstop.

- **The finding that made 128 free, and the reason two predictions were wrong: Lambda grants close to full CPU during the init phase regardless of `MemorySize`.** Measured init was **378 ms at 128 MB and 379 ms at 512 MB** — cold start is effectively memory-independent. Both earlier estimates assumed init scaled with CPU and predicted ~1.6 s and ~6.5 s; both were wrong. **Do not re-litigate 128 on cold-start grounds** without re-measuring; the intuition "less memory means slower cold starts" does not hold for the init phase.
- **A related measurement trap, which cost two false readings:** the first cold start after a fresh image push includes Lambda pulling and caching the container image, and is not comparable to later ones — the 1024 MB figure of 806 ms was that, not a CPU result. And wall-clock timing from `curl` cannot distinguish cold from warm: **only a `REPORT` line carrying `Init Duration` proves a cold start**, and `Memory Size` in that line proves which configuration served it. Two measurements were taken against a configuration that had not finished deploying.
- **Where memory does bite:** handler work went 5 ms → 93 ms between 1024 and 128, about 18×. It is invisible: wall-clock is ~290 ms, dominated by TLS and network to `us-east-2`, so a visitor cannot perceive the difference. The returned artefact is byte-identical.
- **Cost accepted:** headroom drops from 25× to 3.1× over the measured 41 MB. Input is bounded by Lambda's 6 MB request limit and output is ~2.6 KB, so there is no realistic path to exhaustion — but a plan with pathologically many phases renders a large `PLAN.md` in memory. **If a 500 ever appears with no useful log line, memory is the first thing to check.**

- **Kept as planned:** no VPC (a VPC Lambda needs a NAT gateway at ~$32/month, more than the ECS option this whole phase exists to avoid, and Genesis reaches no private resources), and an AWS Budget at $5/month with alerts at 85% actual, 100% actual and 100% forecasted. **The budget's `CostTypes` were set explicitly to `IncludeCredit: false`** — on the default, credits net off the charges, actual spend reads ~$0 while the balance drains, and the alarm stays silent through exactly the scenario it exists to catch. It tracks gross usage instead. The budget lives outside the stack, in the account, so `sam delete` cannot remove it.
- **Scope:** infrastructure
- **Eval hook:** `GET /` returns 501 and `GET /scaffold` returns 405 from a machine that has never authenticated to AWS; `POST /scaffold` returns a zip that installs and passes its own tests; `aws lambda get-function-configuration` reports 128 MB; `aws budgets describe-budget` reports `IncludeCredit: false`


## D-087: The no-UI cut is reopened for one hosted demo page

- **What the cut said:** `CLAUDE.md` lists "a UI" among the things cut from v1, alongside a second template, DynamoDB, mobile and SWE-bench integrations. `docs/phase10-deploy.md` softened it to "one static page, one button, no framework — that reads as a demo affordance rather than an application", and flagged explicitly that this was a judgement call needing a decision.
- **Options:** keep the page to a single keyless scaffold button, clearly inside the cut · reopen the cut and build the full flow · ship the button now and schedule the rest.
- **Choice:** **reopen it.** The page at `https://jborrajo21.github.io/genesis/` gets both a keyless scaffold button using a sample plan **and** the full `/create` flow with multi-round clarifying questions.
- **Reason the cut no longer fits:** it was made when there was nothing to put a UI in front of. There is now a live public API whose most distinctive behaviour — the planner's adaptive questioning — is **invisible from a `curl` example**, because it is a conversation rather than a call. A visitor who will not open a terminal currently sees nothing at all, which makes the deploy a CV line rather than a demonstration. The page is the only way the multi-turn design chosen in D-084 is visible to anyone who is not already reading the tests.
- **Where the new line is**, so this does not become licence for more: one page, no framework, no build step beyond a Pages workflow, no accounts, no persistence, no styling system. It demonstrates the API; it is not a product surface. **A second page, a router, or any state that survives a reload is outside this entry** and needs another one.
- **Cost accepted, and it is not only effort.** A visitor must paste an Anthropic API key into a web page to use the `/create` half — a real trust ask, mitigated only by the key being used for that request and never stored, which the page must state where the field is, not merely in the README. Each planning round is a 15–30 s wait with no streaming, so the page must show that it is working or it will read as broken. And this lands in **the task the cut order says to drop first**, so if the weekend runs out, the scaffold button ships alone and the `/create` flow waits.
- **Why this is an entry rather than a quiet extension:** the decision log is part of what this repo is showing off. A cut list contradicted by the running deployment, with nothing explaining it, is worse than either choice. This is also the second silent scope drift found today — see D-088 — which is the argument for writing it down before building rather than after.
- **Scope:** project scope
- **Eval hook:** the Pages URL loads, scaffolds a sample plan with no key, and completes at least one clarify-then-build round with a key; `CLAUDE.md`'s cut list references this entry rather than contradicting the live site

## D-088: `POST /create` exists, and is a planning round that returns the artefact

- **Context, and an admission.** `docs/phase10-deploy.md` line 129 defines Tier B as "BYOK `/plan` **and `/create`**". Task 1's instructions said "Route `POST /scaffold` and `POST /plan`", and the handler was built to that without anyone noticing the contradiction. **The endpoint was dropped silently, not decided against** — the doc has promised an endpoint the API does not have since Sept 19.
- **Options:** leave it out and correct the tier description · add `POST /create` taking `idea` + `rounds` and returning the zip.
- **Choice:** **add it.** `create` is the product's actual purpose — idea in, buildable repo out. `/plan` and `/scaffold` exist to make the steps inspectable; they are the decomposition, not the headline.
- **The shape, and why it is not a one-shot call.** Over HTTP with multi-turn planning, "create" cannot be a single request: the clarifying questions happen in between. So `/create` is **a planning round that returns the artefact instead of the plan** — identical to `/plan` in request shape and in its `need_info` response, differing only at the end: where `/plan` returns `{"status": "ready", "plan": {...}}`, `/create` scaffolds that plan and returns the zip.
- **Consequence: one endpoint, two response media types.** `application/json` while it is still asking, `application/zip` when it is done. Clients branch on `Content-Type`. This is unusual and is the price of matching the CLI's `create` semantics statelessly; the alternative was forcing a plan with no questions, which D-084 rejected on its own merits.
- **Why not simply let the client compose `/plan` then `/scaffold`:** it can, and that composition is tested and stays supported. `/create` saves a round trip and, more importantly, gives the API the same three verbs as the CLI — a reader who knows `genesis create` finds the endpoint they expect.
- **Cost accepted:** a third endpoint sharing logic with both others, so the scaffold body and the planning round are extracted into helpers used by two callers each. More surface to test and document, for symmetry and one fewer round trip.
- **Scope:** public behaviour
- **Eval hook:** `POST /create` with no key returns 401; with a key and `rounds: []` returns `need_info` questions as JSON; with enough rounds returns `application/zip` that unzips, installs and passes its own tests


## D-089: The demo page is served from GitHub Pages, and `GET /` describes the API

- **Context:** the last two open items in `docs/phase10-deploy.md` — where the static page is served from, and what the Function URL's root returns. They are one decision: the second follows from the first.
- **Options:** serve the page from the Lambda function (same-origin, no CORS) · serve it from GitHub Pages and configure CORS · a custom domain in front of CloudFront.
- **Choice:** **GitHub Pages**, at `https://jborrajo21.github.io/genesis/`, with a `Cors` block on the Function URL. `GET /` on the function returns a **JSON endpoint listing** rather than HTML.
- **This reverses a recommendation made hours earlier the same day, and the reversal is the useful part.** The case for serving from the function was that same-origin removes CORS entirely. True, but CORS here is a `FunctionUrlConfig.Cors` block — five lines of YAML and no handler code, since D-083 had already put CORS in the template rather than in Python. "Removes CORS entirely" sounded decisive and was actually "saves five lines of configuration", which does not outweigh a clean public link.
- **Function URL hostnames cannot be customised.** The generated id is AWS's and there is no setting and no CNAME onto `*.on.aws`. Fronting it is the only way to a readable link, and Pages is the free one; a domain plus CloudFront (~$18/year) is the only option that outlives the AWS account and was not worth it here.
- **`GET /` therefore serves JSON, not HTML.** With the front door on Pages, the Function URL root is the **API root**, so it lists the three endpoints and states the key-handling promise for anyone who arrives by `curl`. A link that 404s is worse than no link, which is why it is not simply unrouted; a test asserts the **whole endpoint set**, so adding a route without documenting it fails CI.
- **`http://localhost:8000` is a permanent allowed origin**, not a temporary one. A dev entry you must remember to re-add is one you will forget. It gives nothing away: **CORS protects a user's ambient credentials**, and this API has none — `/scaffold` is keyless and anyone can `curl` it, while `/plan` and `/create` take a key the caller types in themselves. CORS here is a browser formality, not a security boundary.
- **`ExposeHeaders` is load-bearing and easy to miss.** Cross-origin JavaScript can read only a short safelist of response headers, so `Content-Disposition` and `Genesis-Supported` are invisible to `fetch` unless named. Without it the page cannot name the download or tell the visitor their stack has no template — and it fails **only in a browser**, never in `curl`.
- **Two page-level choices worth recording**, since both look like styling and are not. The API key field is deliberately **not `type="password"`**: a password field makes browsers offer to save the key into the password manager, which interrupts the flow and is bad advice — an API key is not a website login. It is masked with `-webkit-text-security`, which Firefox does not support, so the key stays visible there; that is an accepted trade. And the page **states its scope in the lede** — anything that is not a Python CLI gets a plan without generated code — rather than letting a visitor discover it by typing "a React dashboard". **No roadmap promises appear on the page**: more templates are on the cut list, and advertising them would contradict the decision log.
- **Cost accepted:** two deploy targets to keep in sync, and the CORS origin becomes load-bearing — a mismatch breaks the demo silently from the visitor's side. `site/index.html` also hardcodes the Function URL, so recreating the stack breaks the page quietly; recorded in `docs/open-items.md` (the hardcoded-Function-URL item) rather than solved.
- **Scope:** infrastructure / public behaviour
- **Eval hook:** the Pages URL loads and scaffolds a sample plan with no key; `GET /` returns a JSON listing naming all three endpoints and the key header; a preflight from `http://localhost:8000` is allowed and the real response carries `Access-Control-Expose-Headers`


## D-090: The truncation message stops naming a CLI flag, and the hosted token budget stays fixed

- **Context:** D-039 chose to fail fast on truncation with `PlannerError("response was truncated by max_tokens — increase --max-tokens and try again")`. That message was written when the CLI was the only client. The hosted API has no `--max-tokens`, so a `/plan` or `/create` caller could be handed an instruction they cannot follow.
- **Options:** accept `max_tokens` as a request field so hosted callers get the knob · keep the budget fixed and reword the message so it is actionable for both clients · leave it and document the limitation.
- **Choice:** **keep `_MAX_TOKENS` fixed at 10,000 and reword the message.** It now reads *"the model's response was cut off by the token limit — the idea may be too broad to plan in one pass, or the token budget too low"*: true for both clients, and it names the likelier cause first.
- **Reason — the defect was the message, not the missing knob.** Measured from the Tier 2 corpus, median plan length is **3,210–5,298 tokens across all three models**, so 10,000 is roughly double the largest median and truncation is rare. Adding a request field would mean new public surface, a validation path and a 400 clause, all to fix an error that seldom fires — and it interacts with the function's 60-second timeout, which is the real ceiling on generation length anyway. Fixing the sentence costs one line and closes the whole gap.
- **What was rejected and why it was tempting:** a `max_tokens` body field would have been *consistent* with `model`, which is already a request field. Consistency is a real argument and it loses here to the measurement: `model` exists because the caller pays and has a basis for choosing; nobody choosing a token budget has a basis for it, and the one who needs a bigger one is better served by narrowing the idea.
- **Cost accepted:** a hosted caller who genuinely needs more than 10,000 tokens has no recourse but to split the idea. That is now stated by the error rather than implied by a flag that does not exist for them.
- **Amends D-039**, whose recorded `Choice` quotes the old string. The decision it made — fail fast rather than parse a partial response — is unchanged.
- **Scope:** public behaviour
- **Eval hook:** `grep -r "increase --max-tokens" src/` returns nothing; a `TRUNCATED` stop reason raises a `PlannerError` naming neither a flag nor a client


## D-091: The Ollama adapter raises `AdapterError`, and both adapters export a `DEFAULT_MODEL`

- **Context:** D-084 gave `AnthropicAdapter` typed failures so a rejected key stopped surfacing as `Unexpected error`. `OllamaAdapter` was left raising stdlib `ValueError` and `ConnectionError`, which made the README's "every deliberate failure raises a `GenesisError` subclass" **false on the public front page** — and, more concretely, meant a library consumer catching `GenesisError` missed every Ollama failure. That is the same argument that justified exporting `PlannerError`.
- **Choice:** both `urllib` failures — an HTTP rejection and an unreachable server — now raise `AdapterError(...) from e`, with their messages unchanged. The two now-unreachable `except ConnectionError` clauses in `core.py` are deleted.
- **One error type, not two.** An unreachable server and a rejected request map to the same 502 in the handler, and the CLI already tells them apart by message. A second class would encode a difference nothing acts on.
- **`except ValueError` deliberately stays in `core.py`.** It is not there for Ollama: `_build_adapter` raises it for an unknown adapter name and a missing Anthropic SDK, and `_select_model` raises it for bad menu input. Deleting it alongside the `ConnectionError` clause — the obvious symmetric move — would have routed `Unknown adapter: foo` into the catch-all. **This is the clause-routing hazard that bit three times on Sept 18**, and what made the change safe was that `test_cmd_plan_reports_unreachable_server` and its `cmd_create` twin already assert the Ollama message reaches stderr and that `Could not write output` does not, so a misroute fails loudly.
- **Timed into `0.2.0` on purpose.** It is a **breaking change for library consumers** catching `ValueError` or `ConnectionError` from `OllamaAdapter`. `0.2.0` is already a minor bump carrying a public API expansion, so this is the cheapest moment it will ever be; the same change in `0.2.1` would be a breaking change inside a patch release.
- **Also: `DEFAULT_MODEL` is now a module constant in both adapters**, replacing an inline literal in each signature, and `lambda_handler` imports Anthropic's rather than declaring its own. The argument was **not** symmetry between adapters — `ollama_adapter.py` already had `DEFAULT_BASE_URL` as a module constant while its default model sat inline, so the file was internally inconsistent. A test asserting the handler's default matched the adapter's was written and then deleted: **a test guarding a duplication is worse than not having the duplication.**
- **Rejected: retyping `anthropic_adapter.py:81`** (`ValueError("tool message has no tool_call_id")`). It fires when a consumer hands the adapter a malformed `Message` — argument validation on the library path, which is precisely what `ValueError` means in Python. Converting it would be consistency over idiom. **The README claim was narrowed instead**, to "every failure a user can reach", which is true.
- **Not adopted from Anthropic: an `api_key` parameter.** Ollama is local and has no credential; adding one would be symmetry for its own sake.
- **Scope:** public behaviour
- **Eval hook:** `grep -rn "raise ValueError\|raise ConnectionError" src/genesis/ollama_adapter.py` returns nothing; an unreachable Ollama exits 1 from the CLI with `ollama serve` in stderr and no `Could not write output`


## D-092: The tagged union is possible and is deliberately not shipped

- **Context:** `{"status": "ready"}` with no `plan` is schema-valid and contract-invalid, because `_PLAN_SCHEMA`'s `required` is `["status"]` alone — precisely *because* `questions` and `plan` are alternatives, which a flat object cannot express. Found Sept 14, 2026 by the small-model experiment (D-072): `llama3.2:1b` emitted a bare `ready`; the three-model ladder never has, in 60 plans. D-078 closed the runtime half and deferred the schema half as *"support varies by backend"*. **That phrasing was a guess. This measured it.**

- **Measured Sept 21, 2026, ~60 calls through the real adapters:**
  - **`oneOf` is rejected by Anthropic** — `400 … Schema type 'oneOf' is not supported`. Ollama accepts it.
  - **`anyOf` works on both, and enforces.** Both branches reachable on each backend; **no bare `ready` was produced in any call**. Ollama, told to emit one, was forced to invent a plan it named `'placeholder'` — the grammar refusing to let it stop.
  - **Branch order is a strong prior, not noise.** On a fully-specified idea ending *"Everything is decided; do not ask questions"*: `ready` first → **15/15** plans; `need_info` first → **3/15**, so the model asked anyway 80% of the time. On a vague idea (*"an app"*), **both orders asked, 30/30**. So `ready`-first is **strictly better**, not a trade — it answers a settled idea and still interrogates a vague one.

- **Options:** ship the union with the planner-label block, sharing its eval re-run · ship it in its own block with its own re-run · do not ship it.
- **Choice: do not ship it**, and close the item rather than carry it.
- **Reason — the measurement disqualified the cheap path.** The plan was for the union to ride along with planner-emitted labels and share one re-run. **That only works if the union is a neutral refactor, and it is not:** swinging the ask/plan decision from 20% to 100% is a *generation* change. Bundling it would move two variables at once and make neither result attributable — **D-079's own objection to bundling a fresh idea set into that same run.** So the real choice was "skip it" versus "spend a second eval re-run", and what that re-run would buy is deleting a duplication, for a defect no shipped model has ever produced.
- **Amends D-078.** That entry says the two-place enforcement *"collapses back to one place if the tagged union works"*, which now reads as something imminent. It is not: the union works and is declined. **The duplication is an accepted, standing cost** — a change to the plan contract needs editing both `_PLAN_SCHEMA` and `_round`, and nothing catches you if you forget the second.
- **Consequence for the `steps` validation item:** it was parked as belonging with this work, on the grounds that a stricter grammar might make the runtime check unnecessary. That reasoning is void. It is now a standalone question about runtime validation depth, and it needs deciding on its own terms.
- **Revival conditions**, in the shape D-072 used: (1) a shipped model actually produces a contract-invalid response, or (2) the plan contract is being changed anyway for another reason, so the duplication has to be touched regardless. Then use **`anyOf` with `ready` first**, and budget it its own eval re-run rather than attaching it to one measuring something else.
- **Two findings for `EVAL.md`**, both measured limits of hosted constrained decoding the local backend does not share: Anthropic rejects `oneOf` outright, and **`anyOf` branch order materially changes what is generated, not merely what validates.** Neither is in the API documentation.
- **Scope:** planner contract
- **Eval hook:** none — a decision not to change code. The evidence is the numbers above; re-running `scratch.py` reproduces them.


## D-093: The planner stays honest, and `Plan.supported` stays as a derived cache

- **Context:** the two decisions `docs/template-registry.md` left open, settled before the planner-label block opens so it starts with nothing undecided. Both are about what the label changes and what it must not.

### The planner is not biased toward what Genesis can scaffold

- **Options:** steer `_PLANNING_INSTRUCTION` toward Python CLIs so more plans are scaffoldable · leave the planner recommending whatever is idiomatic and let the registry refuse what it cannot build.
- **Choice:** **honest.** The planner recommends the right stack for the idea; the registry decides separately whether a template exists.
- **Reason — biasing it would delete the project's headline result rather than improve it.** The measured finding is that end-to-end success *anti-correlates* with model capability (local 8B 90%, Haiku 70%, Sonnet 70%) **because** stronger models reach for the idiomatic ecosystem: Node for a git commit linter, JS tooling for a static site generator. Steering toward Python would raise the success number while destroying the thing that number means. The constraint being violated is ours, not the model's, and a metric that hides that is worse than one that reports it.
- **It also protects a claim the README already makes:** *"a stack it can't scaffold still gets a plan plus a manual checklist rather than a refusal."* That is a promise to stay useful when Genesis cannot help. A planner nudged toward its own template would be a **worse planner pretending to be a better scaffolder** — and the person receiving a mediocre Python design for a problem that wanted Node would never know.
- **Consistent with the standing rule** that `_PLANNING_INSTRUCTION` is not tuned in response to eval results. This decides the same thing up front rather than after seeing numbers, which is the only time it can be decided honestly.
- **Cost accepted:** end-to-end success stays capped by how often the model independently picks a stack we cover, and that ceiling is visible in the published numbers. Raising it is the second template's job, not the planner's.

### `Plan.supported` stays, as a cache that is always recomputed

- **Options:** drop the field and have callers ask the registry · keep it on the dataclass.
- **Choice:** keep it. **Neither option involves a model** — the only LLM contribution is emitting the label, and selection stays deterministic and keyless (D-028/D-063).
- **Reason:** with a label, `supported` becomes a dict lookup, so caching buys nothing computationally — what it buys is **presence in the serialised plan**. `/plan` returns `asdict(plan)`, so a hosted caller can see whether they will get code before asking for it, and `/scaffold` reads it for the `Genesis-Supported` header. Dropping it would move that cost onto every consumer.
- **The staleness objection is already answered in code and is now written down: `_parse_plan` ignores `supported` on input and always recomputes it.** It is a cache with a recompute rule, not a source of truth, and a user-supplied plan file claiming `"supported": true` cannot lie its way past the registry. The label change makes this cleaner rather than riskier — `supported` stops being a guess about a joined string and becomes a lookup on a declared fact.
- **Scope:** planner contract and public behaviour
- **Eval hook:** a plan file with `"supported": true` and an unsupported stack still scaffolds generically; `_PLANNING_INSTRUCTION` contains no instruction to prefer Python or CLIs


## D-094: The planner-emitted label is `{language, kind}`, free-form, trusted, and optional

- **Context:** `select_template()` joins the stack into one lowercased string and refuses the template if any exclude marker appears anywhere in it. Measured Sept 21, 2026 against the live registry, the failure is wider than "the Flask veto" implies: `['Python 3.11', 'requests', 'beautifulsoup4 for web scraping']`, `['Python 3.11', 'click', 'a GUI is out of scope']` and `['Python 3.11', 'argparse', 'no web framework needed']` are all refused. **A plan is refused for saying it is *not* a web app.** No keyword rule fixes that — you cannot match your way out of a negation without parsing English.

### Shape: `{"language": "python", "kind": "cli"}`, with `kind` free-form

- **Options:** `{language, kind}` with `kind` free · the same with `kind` constrained to a schema enum · `{frontend, backend, database}` nullable · `components: [{role, language}]`.
- **Choice:** `{language, kind}`, `kind` **not** enumerated in the schema. The taxonomy is steered by the prompt, not frozen by the grammar.
- **Rejected — `{frontend, backend, database}`: it is a *web* ontology.** A CLI is `{null, null, null}` under it, and so is a library, a data pipeline and a game. It makes the common case unrepresentable to accommodate a case that is on the cut list.
- **Rejected — a `components` list.** It is the honest generalisation if Genesis ever composes templates, and it does not assume the web. But **one plan still maps to one template**: a React + FastAPI + Postgres plan is not scaffoldable by composing three templates, that is a monorepo generator and a different product. The list would describe something nothing can act on.
- **The criterion was not "is it rich enough" but "does adopting something richer later force a migration or is it additive".** Every label change alters generation and therefore costs an eval re-run, so extensions are not free here the way they are in ordinary software. Adding `components` beside `{language, kind}` later is additive; moving off `{frontend, backend, database}` would be a migration, paid for with a re-run spent on a shape then abandoned.
- **Free-form rather than an enum** because an enum freezes the taxonomy at whatever can be imagined today and makes every addition a schema change plus a re-run — and an idea fitting none of the listed kinds would be *mislabelled* rather than honestly unmatched, which is the current failure relocated. An unseen `kind` simply finds no template and falls through to the generic scaffold: the behaviour the eval rates most reliable (100% on the unsupported half, every model, both runs).

### The label is trusted; disagreement is measured, not enforced

- **`docs/template-registry.md` sketched a cross-check** against the stack text, on the grounds that *"a model can write `FastAPI` and label it `cli`"*. **That is reversed here.** A cross-check would reinstate keyword matching against the stack — the exact mechanism producing today's false negatives — and would re-merge the metric D-028/D-063 keep apart: *did the model pick a stack we cover* must stay distinguishable from *did we read that stack correctly*.
- **Instead the eval gains a number:** how often the label contradicts the stack text, computed from the saved corpus without affecting behaviour. If mislabelling turns out to be common, it gets fixed then, with evidence. The measured prior is encouraging — the old classifier was right in 85–95% of cases across both runs, and the failure was stack *choice*, not classification.

### A missing label means no match

- `_parse_plan` serves the planner **and** `genesis scaffold plan.json`, and every plan file saved before this change has no label. **No label → no template → generic scaffold**, the existing unsupported path.
- **Cost accepted, and it is a visible regression:** a hand-written Python CLI plan that used to scaffold real code now yields `PLAN.md` and a README. The alternative — falling back to positive markers only when the label is absent — would keep those working and would even drop the excludes that cause every measured false negative, but it keeps a second selection path alive permanently and is wrong in the too-generous direction: an unlabelled Python + Flask plan would get a CLI scaffold. One path that refuses honestly beats two paths that disagree.
- **Scope:** planner contract, template selection, eval
- **Eval hook:** the six stacks in `docs/open-items.md` A1 classify correctly; a plan file with no label scaffolds generically rather than raising; `select_template` contains no string matching against the stack
